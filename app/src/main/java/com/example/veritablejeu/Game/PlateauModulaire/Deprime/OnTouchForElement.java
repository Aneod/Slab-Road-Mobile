package com.example.veritablejeu.Game.PlateauModulaire.Deprime;
...
public abstract class OnTouchForElement {
    ...
    public OnTouchForElement(ModularObject modularObject) {
        setOnTouchForElement(modularObject);
    }
    ...
    private void setOnTouchForElement(ModularObject modularObject) {
        if(modularObject == null) {
            return;
        }

        BoardsMovements onToucheListener = modularObject
                .getGame().getOnToucheListenerPlateauModulaire();

        modularObject.setOnTouchListener(new View.OnTouchListener() {
